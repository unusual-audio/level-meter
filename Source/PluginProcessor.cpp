...
void LevelMeterAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    level.reset(sampleRate, .3f);
    level.setCurrentAndTargetValue(juce::Decibels::decibelsToGain(minLevel));
}

float LevelMeterAudioProcessor::getLevel()
{
    return juce::Decibels::gainToDecibels(level.getCurrentValue(), minLevel);
}

float LevelMeterAudioProcessor::getMinLevel()
{
    return minLevel;
}
...
void LevelMeterAudioProcessor::processBlock (juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    ...
    sum.setSize(1, buffer.getNumSamples());
    sum.clear();
    for (int i = 0; i < getTotalNumInputChannels(); i++)
    {
        sum.addFrom(0, 0, buffer, i, 0, buffer.getNumSamples());
    }
    
    level.skip(sum.getNumSamples());
    
    float e = sum.getMagnitude(0, 0, sum.getNumSamples()) / 2;
    if (e < level.getCurrentValue())
        level.setTargetValue(e);
    else
        level.setCurrentAndTargetValue(e);
}
